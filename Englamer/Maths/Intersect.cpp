...
{
	glm::vec3 e1min = ((Transform*)e1->get_component(TRANSFORM))->position + (((Collider*)e1->get_component(COLLIDER))->get_aabb_min() * ((Transform*)e1->get_component(TRANSFORM))->scale);
	glm::vec3 e2min = ((Transform*)e2->get_component(TRANSFORM))->position + (((Collider*)e2->get_component(COLLIDER))->get_aabb_min() * ((Transform*)e2->get_component(TRANSFORM))->scale);
	glm::vec3 e1max = ((Transform*)e1->get_component(TRANSFORM))->position + (((Collider*)e1->get_component(COLLIDER))->get_aabb_max() * ((Transform*)e1->get_component(TRANSFORM))->scale);
	glm::vec3 e2max = ((Transform*)e2->get_component(TRANSFORM))->position + (((Collider*)e2->get_component(COLLIDER))->get_aabb_max() * ((Transform*)e2->get_component(TRANSFORM))->scale);
